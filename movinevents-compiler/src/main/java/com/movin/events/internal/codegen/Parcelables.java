package com.movin.events.internal.codegen;
...
class Parcelables {
    ...
    static void readValue(CodeBlock.Builder block, DispatcherProcessor.Property property, final TypeName parcelableType) {
...
//            block.add("($T) in.readHashMap($T.class.getClassLoader())", property.typeName, parcelableType);
    ...
    public static void readValueWithTypeAdapter(CodeBlock.Builder block, DispatcherProcessor.Property property, final FieldSpec adapter) {
        if (property.isNullable()) {
            block.add("in.readInt() == 0 ? ");
        }
    ...
    public static CodeBlock writeValue(DispatcherProcessor.Property property, ParameterSpec out, ParameterSpec flags, Types typeUtils) {
        CodeBlock.Builder block = CodeBlock.builder();
...
//            block.add("$N.writeMap($N)", out, property.fieldName);
    ...
    public static CodeBlock writeValueWithTypeAdapter(FieldSpec adapter, DispatcherProcessor.Property p, ParameterSpec out) {
        CodeBlock.Builder block = CodeBlock.builder();
    ...
    static TypeName getTypeNameFromProperty(DispatcherProcessor.Property property, Types types) {
