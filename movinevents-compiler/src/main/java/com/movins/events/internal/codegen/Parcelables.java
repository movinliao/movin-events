...
class Parcelables {
    ...
    static void readValue(CodeBlock.Builder block, AutoDispatcherProcessor.Property property, final TypeName parcelableType) {
...
//            block.add("($T) in.readHashMap($T.class.getClassLoader())", property.typeName, parcelableType);
    ...
    public static void readValueWithTypeAdapter(CodeBlock.Builder block, AutoDispatcherProcessor.Property property, final FieldSpec adapter) {
        if (property.isNullable()) {
            block.add("in.readInt() == 0 ? ");
        }
    ...
    public static CodeBlock writeValue(AutoDispatcherProcessor.Property property, ParameterSpec out, ParameterSpec flags, Types typeUtils) {
        CodeBlock.Builder block = CodeBlock.builder();
...
//            block.add("$N.writeMap($N)", out, property.fieldName);
    ...
    public static CodeBlock writeValueWithTypeAdapter(FieldSpec adapter, AutoDispatcherProcessor.Property p, ParameterSpec out) {
        CodeBlock.Builder block = CodeBlock.builder();
    ...
    static TypeName getTypeNameFromProperty(AutoDispatcherProcessor.Property property, Types types) {
    ...
    public static TypeName getParcelableType(Types types, TypeElement type) {
        ...
        while (typeMirror.getKind() != TypeKind.NONE) {
            ...
            TypeName typeName = TypeName.get((Type) typeMirror);
